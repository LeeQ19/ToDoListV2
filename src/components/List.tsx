...
import { ICard, IList } from "./interface";
...
function List({ id, name, cards, editList, deleteList }: IList & { editList: (list: IList) => void, deleteList: (id: number) => void }) {
  ...
  useEffect(() => {
    if (cards !== currCards) {
      editList({ id: id, name: name, cards: currCards });
    }
  }, [currCards]);

  const addCard = (value: string) => {
    setCurrCards((v) => [...v, { id: v.length, text: value }]);
  };

  const editCard = (card: ICard) => {
    setCurrCards(v => v.map(w => w.id === card.id ? card : w));
  };
