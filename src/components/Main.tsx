...
const Option = styled.li<{ value: number }>`
...
function Main() {
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    if (type === "list") {
      const listsResult = boards[id].lists.map(v => ({ id: v.id, name: v.name, cards: v.cards }));
      const [minIndex, maxIndex] = [source.index, destination.index].sort();
      const dragged = listsResult.splice(source.index, 1)[0];
      listsResult.splice(destination.index, 0, dragged);
      listsResult.slice(minIndex, maxIndex + 1).forEach((v, i) => { v.id = minIndex + i });
      setBoards(v => v.map(w => w.id === id ? { id: w.id, name: w.name, lists: listsResult } : w));
    } else if (type === "card") {
      const sdid = parseInt(source.droppableId.split("-")[1]);
      const ddid = parseInt(destination.droppableId.split("-")[1]);
      console.log(`From ${source.droppableId} - ${source.index} To ${destination.droppableId} - ${destination.index}`);
      if (sdid === ddid) {
        const cardsResult = boards[id].lists[sdid].cards.map(v => ({ id: v.id, text: v.text }));
        const [minIndex, maxIndex] = [source.index, destination.index].sort();
        const dragged = cardsResult.splice(source.index, 1)[0];
        cardsResult.splice(destination.index, 0, dragged);
        cardsResult.slice(minIndex, maxIndex + 1).forEach((v, i) => { v.id = minIndex + i });
        setBoards(v => v.map(w => w.id === id ? { id: w.id, name: w.name, lists: w.lists.map(x => x.id === sdid ? { id: x.id, name: x.name, cards: cardsResult } : x) } : w));
      } else {
        const cardsSource = boards[id].lists[sdid].cards.map(v => ({ id: v.id, text: v.text }));
        const cardsDestination = boards[id].lists[ddid].cards.map(v => ({ id: v.id, text: v.text }));
        const dragged = cardsSource.splice(source.index, 1)[0];
        cardsSource.slice(source.index).forEach((v, i) => { v.id = source.index + i });
        cardsDestination.splice(destination.index, 0, dragged);
        cardsDestination.slice(destination.index).forEach((v, i) => { v.id = destination.index + i });
        setBoards(v => v.map(w => w.id === id ? { id: w.id, name: w.name, lists: w.lists.map(x => x.id === sdid ? { id: x.id, name: x.name, cards: cardsSource } : x.id === ddid ? { id: x.id, name: x.name, cards: cardsDestination } : x) } : w));
      }
    }
  };
